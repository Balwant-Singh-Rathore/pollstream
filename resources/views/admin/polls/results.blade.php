    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="mx-auto max-w-2xl">
                ...
                <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                    ...
                    <div class="flex items-center gap-1.5 text-white/40">
                        <p class="mt-1 text-sm text-gray-500" id="totalVotes">
                            {{ $poll['total_votes'] ?? 0 }} total votes
                        </p>
                        <span id="plusOneAnimation"
                            class="right-0 -top-2 text-green-400 text-lg font-bold opacity-0 pointer-events-none">
                            +1
                        </span>
                    </div>
                    ...
                    <div class="mt-6 space-y-4">
                        ...
                        @foreach ($poll['options'] as $opt)
                            @php
                                $pct = $poll['total_votes'] > 0 ? round(($opt['votes_count'] / $poll['total_votes']) * 100) : 0;
                            @endphp

                            <div>
                                <div class="mb-1 flex items-center justify-between text-sm">
                                    <span class="font-medium text-gray-900">{{ $opt['option_text'] }}</span>
                                    <span class="text-gray-500" id="voteCount-{{ $opt['id'] }}">
                                        {{ $opt['votes_count'] }} votes · {{ $pct }}%
                                    </span>
                                </div>

                                <div class="h-2.5 w-full overflow-hidden rounded-full bg-gray-200">
                                    <div class="h-full rounded-full bg-[#6366f1] transition-all duration-500"
                                        style="width: {{ $pct }}%" id="progress-{{ $opt['id'] }}">
                                    </div>
                                </div>
                            </div>
                        @endforeach
                    ...
                    <div class="mt-6 flex items-center gap-2 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2">
                        ...
                        <button onclick="copyLink()" id="copyLinkBtn"
                            class="rounded-md p-1.5 text-gray-500 hover:bg-gray-200">
                            Copy
                        </button>
                    </div>
            ...
            <div
                ...
                data-options='@json($poll->options)'
            ></div>
            <script>
                function copyLink() {
